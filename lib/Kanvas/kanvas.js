...
;var Kanvas = Kanvas || (function (doc, undefined) {
  ...
  if (shimPath) {
    ...
    kanvasPathProto._curveSeg = function (x0,y0, cp1x,cp1y, cp2x,cp2y, x,y, dontsplit) {
      var state = this._state;
      var segs = this._segs;
      var p = segs.length;
      ...
      kanvasPathProto.strokePath = function (styles, transformation) {
        ...
        for (var i = 0; i < segs.length;) {
          ...
          if (npoint === 2) {
            ...
            for (var j = 1; j < npoint; j += 3, i += 6, p += 6) {
              var cp1x = segs[i + 3];
              var cp1y = segs[i + 4];
